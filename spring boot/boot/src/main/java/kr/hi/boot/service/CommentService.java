...
public class CommentService {
	...
	public PageMaker getPageMaker(int poNum, Criteria cri) {
		//한페이지네이션에서 최대 페이지 수를 3개로 선언
		int pageCount = 3;
		//다오에게 게시글번호와 현재페이지 정보를 주면서 전체 댓글 수를 가져오라고 요청
		//전체댓글수 = 다오야.전체댓글수가져와(게시글번호,현재페이지정보);
	...
	public String insertComment(Comment comment, CustomUser user) {
		...
		//다오에게 게시글 번호, 댓글 내용, 사용자아이디을 주면서 게시글을 등록하라고 요청 후 등록 여부를 알려달라고 요청
	...
	public String deleteComment(int coNum, CustomUser user) {
		//사용자가 로그인되어 있지 않으면 "로그인이 필요한 서비스입니다."라고 반환
		if(user == null) {
			return "로그인이 필요한 서비스입니다.";
		}
		...
		//다오에게 댓글번호를 주면서 삭제하고 결과를 알려달라고 요청
		//결과 = 다오야.댓글삭제해(댓글번호);
	...
	private boolean isWriter(int coNum, String id) {
		//다오에게 댓글번호 주면서 댓글정보 주면서 가져오라고 요청
		//댓글 = 다오야.댓글 가져와(댓글번호);
		Comment comment = commentDAO.selectComment(coNum);
		//가져온 댓글정보가 없으면 false를 반환
		if(comment == null) {
			return false;
		}
