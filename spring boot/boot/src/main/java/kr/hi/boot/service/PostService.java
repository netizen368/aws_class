...
public class PostService {
	...
	public void delecteBoard(int num) {
	...
	public PageMaker getPageMaker(Criteria cri) {
		...
		//다오에게 현재 페이지 정보를 주면서 페이지 정보와 일치하는 게시글 전체의 수를 가져오라고 요청
		//게시글수 = 다오야.게시글전체수를 가져와(현재페이지정보)
		int count = postDAO.selectPostListCount(cri);
		
		//최대 페이지수, 총 게시글 수, 현재 페이지정보를 이용하여 PageMaker클래스 객체를 생성 후 반환
		//PageMaker pm = new PageMaker(한 페이네이션의 최대 페이지수, 현재페이지정보, 현재 페이지 정보에 맞는 전체 컨텐츠 수);
