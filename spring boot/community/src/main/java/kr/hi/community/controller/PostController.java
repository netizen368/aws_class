...
import kr.hi.community.serivce.PostService;
...
public class PostController {
	...
	*/
	...
	public String postDetail(
		...
		//기본키니까 => 기본키의 정의 => 기본키로 검색하면 최대 1행이 조회되는게 컬럼
		...
		model.addAttribute("user", customUser == null? "" : 
	...
	public String postUpdate(
		...
		//서비스에게 게시글 번호(postNum)을 주면서 게시글을 가져오라고 요청
		//- 게시글 객체 = 서비스.메서드명(게시글번호);
		//- 게시글에 해당하는 클래스 : PostVO
		//- 게시글 번호 : postNum
		//- 메서드명은 적당히 작성하면 되믄데, 이미 구현된 getPost를 가져오면 새로 추가하지 않아도 됨
		//- 객체명은 적당히 작성하면 됨
		PostVO post = postService.getPost(postNum);
	...
	public String postUpdatePost(
		...
		*/
		//화면에서 title과 PostDTO에 멤버변수 title이 이름이 같기 때문에
