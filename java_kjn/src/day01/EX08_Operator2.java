...
public class EX08_Operator2 {
	...
	public static void main(String[] args) {
		...
		// 정답은 &&. 성적이 90이상(이고) 100이다라고 표현
		
		//졸업하려면 토익이 600이상이거나 영어회화 과목을 이수해야한다.
		//정답은 ||. 토익이 600이상(이거나) 영어회화 과목을 이수해야한다.
		int score = 95;
		//이상은 크거나 같다. 이하는 작거나 같다로 표현
		System.out.println(score + "점은 A?" + (score >= 90 && score <= 100));
		
		int toeic = 650;
		boolean passEglish = false;//영어 회화 이수 여부
		System.out.println("토익 성정 : " + 600);
		System.out.println("영어 회화 이수 : " + passEglish);
		System.out.println("졸업이 가능?" + (toeic >= 600 || passEglish));
		 ...
		 *     - 4 << 3 : 4 * 2^3 => 4 * 8 => 32
		 *   - a >> n : n비트씩 오른쪽으로 이동, 2^n 나눈 효과. 이동 후 빈왼쪽은 부호 비트로 채움
