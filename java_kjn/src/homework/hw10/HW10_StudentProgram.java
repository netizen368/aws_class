...
public class HW10_StudentProgram {
	...
	public static void main(String[] args) {
		...
		String studentsFillName = "student.txt";
		String subjectsFillName = "subejct.txt";
		try {
			students = (ArrayList<Student>)load(studentsFillName);
		}catch(Exception e) {
			println("학생 목록 정보 불러오기 실패");
			students = new ArrayList<Student>();
		}
		try {
			subjects = (ArrayList<Subject>)load(subjectsFillName);
		}catch(Exception e) {
			println("과목 목록 정보 불러오기 실패");
		...
		do {
			...
			case EXIT: 
				exit();
				save(studentsFillName, students);
				save(subjectsFillName, subjects);
				break;
			default:
	...
	private static void save(String fillName, Object obj) {
		 
		try(ObjectOutputStream oos =
				new ObjectOutputStream(new FileOutputStream(fillName))){
			oos.writeObject(obj);
			oos.flush();
		}catch(Exception e) {
			println("저장에 실패했습니다.");
	...
	private static void addSubjectScore(List<Student> students, List<Subject> subjects) {
		...
		//아래 코드는 생성자를 추가해서 위 코드 대신 추가 할 수 있음
		//SubjectScore subjectScore = new SubjectScore(subject, score);
		
		//없는 과목이면 등록되지 않은 과목입니다를 출력 후 종료  
		if(!subjects.contains(subject)) {
			println("등록되지 않은 과목입니다.");
			return;
		}
		...
		//List에 있는 과목이면 수정 후 성적을 수정했습니다. 출력
		if(isAdd) {
			println("성적을 추가했습니다.");
		}
		//실패하면 성적을 수정했습니다를 출력
	...
	//학년, 학기, 과목명을 입력받아 과목 정보를 삭제하는 메서드
	private static void removeSubject(List<Subject> subjects) {
		
		//학년, 학기, 과목명을 입력
		Subject subject = inputSubject();
		//일치하는 정보가 있으면 삭제 후 알림(과목이 삭제되었습니다.)
	...
	private static void removeStudent(List<Student> students) {
		...
		//학생 정보를 이용하여 삭제하는데 학생 정보가 없으명
		//일치하는 학생이 없습니다. 안내문구 출력 후 종료
	...
	private static void searchStudent(List<Student> list) {
		...
		for(Student tmp : list) {
			//tmp에 학년, 반, 번호가 student의 학년,반,번호와 같으면 
	...
	//searchStudent2는 Student 클래스를 equalls를 오버라이딩하여 이용 
...
//		String name = scan.next();
	...
	private static void addStudent2(List<Student> list) {		
		...
		int index = list.indexOf(student);
		//있는 학생이면 이미 등록된 학생입니다.라고 출력 후 종료 
		if(index >= 0) {
