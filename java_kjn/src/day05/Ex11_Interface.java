...
public class Ex11_Interface {
	...
	public static void main(String[] args) {
		 ...
		 * - 기능 목록만 나열
		 * - 인터페이스에서 변수를 선언하면 앞에 자동으로 public final static이 추가됨
		 *   => 반드시 초기화를 해야 함
		 * - 인터페이스에서 메서드를 선언하면 앞에 자동으로 public abstract가 추가됨
		 * - 인터페이스 선언하기
		 *  interface 인터페이스명{
		 *  	추상메서드;
		 *  	클래스 상수;
		 ...
		 * - 구현 클래스 선언하기
		 *  class 클래스면 implements 인터페이스명1, 인터페이스명2,...{
		 *  	//추상 메서드 오버라이딩
		 *  }
		 * - 왜 사용?
...
interface A{
	...
	//작업2 진행 여부에 따라 직업3이 진행되고 결과를 마무리 함
 ...
 * 이 때 인터페이스가 있으면 3번은 1번과 2번이 끝낼때까지 기다릴 필요없이 구현할 수 있음
 */
interface BaseBall{
