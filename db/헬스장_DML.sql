...
INSERT INTO CLASS(NAME, CAPACITY, FEE, TRAINER_ID) VALUES 
('모닝 요가', 20, 100000, 2), ('지옥의 크로스핏', 15, 150000, 3);

# 수강 신청 
# - 이순신(1) 회원이 모닝요가(1)와 지옥의 크로스핏(2)을 수강 신청 후 결제 완료 함
# - 성춘향(2) 회원이 모닝요가(1)를 수강 신청함 
#   결제완료하면 결제, 수강신청만 하면 신청으로 관리
INSERT INTO ENROLLMENT(PAYMENT_STATE, MEMBER_ID, CLASS_ID) VALUES 
('결제', 1, 1), # 이순신 회원이 모닝 요가를 수강 신청 후 결제
('결제', 1, 2), # 이순신 회원이 지옥의크로스핏을 수강 신청 후 결제
('신청', 2, 1); # 성춘향 회원이 모닝요가를 수강 신청 

# 이순신(1) 회원이 지옥의 크로스핏(2) 강좌를 출석체크 함(현재시간)
INSERT INTO ATTENDANCE(MEMBER_ID, CLASS_ID)
VALUES(1, 2); 

# 새로운 강좌를 등록
# - 1강좌명 : 크로스핏 초급반, 정원 : 30명, 수강료 : 50,000
# 담당 : 김철수(1)
