# 트리거
# - 테이블에 대한 이벤트(insert, update, delete)에 반응해 자동으로 시행되는 작업
# - A테이블에 insert 트리거를 등록하면, A테이블에 데이터가 추가되어(insert)
#   이벤트가 발생하면 트리거가 동작
# - 트리거를 통해 데이터 무결성을 지킬 수 있음
...
# - BUY 테이블에 INSERT 트리거를 등록한 후, 트리거에서 BUY 테이블을 수정하려고 하면 안됨 
