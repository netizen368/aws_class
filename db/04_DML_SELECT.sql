...
# 1학년 1반 학생을 조회 # 별명에 공백이 들어가는 경우 ,를 붙여야함
...
# 컬럼 not between A and B : A미만 또는 B초과 => 컬럼 < A and 컬럼 > B
...
# 각 과목별 평균을 조회
select sj_code 과목번호, avg(score) 평점 from score group by sj_code;
...
select grade, semester, count(title) 과목수 from subject 
where grade = 1 and semester = 1; 

# 각학년별, 학기별 들어야할 과목수를 조회하는 쿼리
select grade, semester, count(title) 과목수 from subject 
...
from score 
# where avg(score) >= 90 # 실행 시 에러발생. 집계함수는 where절에서 사용 X
group by sj_code
...
# JOIN
# 2개 이상의 테이블을 묶어서 하나의 결과를 만들 떄 사용
...
# 두 테이블을 연결한 후 연결할 데이터가 없는 쪽는 null처리
# left join/ right join : 왼쪽/오른쪽 테이블을 기준으로 연결

# left join : 테이블2을 기준으로 테이블2 옆에 테이블1를 이어 붙임
# select * from 테이블1
# left join 테이블2 on 테이블1.컬럼1 = 테이블2.컬럼2;

# right join : 테이블1을 기준으로 테이블1 옆에 테이블2를 이어 붙임
# select * from 테이블1
...
GROUP BY studnet.code;

# 학생의 성적을 조회(학년,반,번호,이름, 학년,학기,과목,성적) => 등록된 모든 학생이 조회
...
left join subject on st_code = subject.code;