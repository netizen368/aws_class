...
# 방법2 : 학생전공을 JOIN하여 학생 전공이 160인 학생을 조회
SELECT * FROM STUDENT 
	JOIN STUDENT_MAJOR ON ST_NUM = SM_ST_NUM
    WHERE SM_MJ_CODE = 160;
    
SELECT STUDENT.* FROM STUDENT_MAJOR
	LEFT JOIN STUDENT ON SM_ST_NUM = ST_NUM
    WHERE SM_MJ_CODE = '160';
...
SELECT MAJOR.*, COUNT(SM_NUM) 학생수 FROM MAJOR
	LEFT JOIN STUDENT_MAJOR ON MJ_CODE = SM_MJ_CODE
    GROUP BY MJ_CODE;

# 홍길동(2025160001) 학생이 수강(수강신청)한 모든 강의를 조회
# 강의제목 포함
...
FROM 
	(SELECT * FROM COURSE
		WHERE CO_SCORE IS NOT NULL
			AND CO_SCORE NOT IN("F", "FAIL")# 성적이 있는 강의를 조회
    ) C
	JOIN LECTURE ON CO_LT_NUM = LT_NUM
    JOIN SUBJECT ON SJ_CODE = LT_SJ_CODE
...
# 강의별 수강생 수가 가장 많은 강의를 조회(여러개 조회)
SELECT * FROM (
	SELECT
...
SELECT SJ_NAME, LT_CURRENT 수강생수 FROM LECTURE
JOIN SUBJECT ON SJ_CODE = LT_SJ_CODE
WHERE
	LT_CURRENT = (SELECT MAX(LT_CURRENT) FROM LECTURE);
