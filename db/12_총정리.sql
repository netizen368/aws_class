...
DDL (2번 파일)
 ...
 - CREATE TABLE IF EXISTS 테이블명(
   컬럼1 타입 옵션,
   컬럼2 타입 옵션,
   CONSTRAIT 제약명 PRIMARY KEY(컬럼),
   CONSTRAIT 제약명 FORIEGN KEY(컬럼) REFERENCES 테이블명(기본키명) 
   )
   - 옵션 : PRIMARY KEY, NOT NULL, UNIQUE, DEFALUT 초기값 
		   AUTO_INCREMENT(기본키에만) 
   - CONSTRAINT 제약명은 생략가능. 
   - 자동으로 만들어서 추가됨.
...
DML (3번 파일)
 ...
 - SELECT (4번 파일)
   ...
   - 실행 순서
    ...
    > HAVING 
    > SELECT * 
     (단, 집계함수인 경우 MYSQL과 오라클은 편의를 위해 별칭을 다른 곳에서 사용할 수 있음) 
    > ORDER BY 
    > LIMIT
   - 서브 쿼리
     - 쿼리 안에 쿼리가 들어감
...
# 스케줄러 (11번 파일)
# - 주기적으로 실행해야 하는 코드를 예약하거나 특정 시간에 한번만 실행해야하는 코드를 
# - 예약할 때 사용 
