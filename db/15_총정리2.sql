...
1. 사용자 요구 도출/정의 
2. 개념적 모델 정의(ERD) 
3. 논리적 모델 정의 : 릴레이션을 저의하고, 릴레이션의 속성들을 정의  
...
- 내장함수
  ...
  - 조건문 관련 
- JOIN
  - 두 개 이상 테이블을 연결할 때 사용
  - INNER JOIN
    - 두 테이블에 데이터가 있는 행들만 연결 
  - OUTER JOIN
    - 한 테이블을 기준으로 연결. 데이터가 없으면 없는데로 연결 
    - LEFT : 왼쪽 테이블 기준
    - RIGTH : 오른쪽 테이블 기준 
- 서브쿼리
  - 쿼리 안에 쿼리가 들어가는 것
...
- TCL
  - 트랜잭션 제어어 
  - 트랜잭션 시작, 저장, 롤백 
  - START TRANSACTION, SAVEPOIN, ROLLBACK
...
정규화 
- 데이터 중복을 최소화하여 무결성을 유지하기 위해 데이터를 구조화하는 것
  => 테이블 쪼개기 
- 제 1정규화
  - 값이 원자값이 되도록 테이블로 나눔 
- 제 2정규화
...
# TRUNCATE VS DROP VS DELETE 처리를 확인하는 예제 
...
INSERT INTO TEXT1(NAME) VALUES('TEST1'), ('TEST2');
INSERT INTO TEXT2(NAME) VALUES('TEST1'), ('TEST2');
INSERT INTO TEXT3(NAME) VALUES('TEST1'), ('TEST2');
